...
pars = {'V_th': -50.0, 'V_reset': -70.0, 'g_L': 25.0, 'C_m': 500,  'V_init': -70.0, 'E_L': -70.0, 'tref': 5.0, 'dt': 0.1,
        'tm':5, 'tp':3, 'Ap':0.6, 'Am':-0.3, 'range_t': np.arange(0, 1000, 0.1), 'Iinj': 3.5, 'lr': 1, 'stdp_rate': 0.0625}
...
snn = m.SNN([layer1], input, Lt, 1, range_t, rate=rate, nu=nu, time_step=time_step, train=False, dVs=0, check=True, file_name = f'output_data/data_1000_{0}.csv')
...
for i in range(1, 20):
        ...
        start_V = V[-1, 0]
        layer1 = m.Layer(1, 1, start_V, neuron)
        snn = m.SNN([layer1], input, Lt, 1, range_t, rate=rate, nu=nu, time_step=time_step, train=False, dVs=dV, check=True, file_name = f'output_data/data_1000_{i}.csv')
        in_spikes, out_spikes, out_rate, V = snn.forward()

'''
#plot files
V, out_spikes, in_spikes, dV = utils.get_csv_data("plot")
range_t = np.arange(0, 20000, 0.1)
utils.plot_spikes(1, 1, in_spikes*Iinj, out_spikes.T, V, range_t, V_th, E_L, legend = [f"{nu}", f"{time_step}", f"{rate}", f"{alfa}"])